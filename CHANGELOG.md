...
### Added
- **`mbgs_request_home_url` bridge filter** — server-side consumers (e.g. a plugin sending an `Origin` payload to an external API) can request the home URL as seen from the browsed Brand domain: `apply_filters( 'mbgs_request_home_url', home_url() )`. Substitution requires the request's Host+path to have explicitly matched a configured Brand URL rule with URL rewrite enabled (scheme honors force-https) — the default-Brand fallback and the admin `?mbgs_preview_brand` override never trigger a swap, since neither reflects an operator-configured rule for the browsed Host. With no rule match or the plugin inactive, the input passes through unchanged. The substituted authority derives from the client-supplied Host header; it is only ever a host that explicitly matched a configured Brand URL rule — consumers must still treat it as untrusted for anything beyond link/branding selection. First consumer: aucteeno-nexus, whose password-reset/registration `Origin` previously always carried the canonical host, producing wrong-brand emails from Brand domains.
