...
- **`@wp-playground/cli`'s Blueprint `"login": true` step 302-redirects-to-self on every request from a client that doesn't already carry the cookie it sets on that first hit.** Confirmed empirically: a cookie-less client (plain `curl`) loops forever; a cookie-jar client resolves in exactly 2 hops (`302` then `200`). A real browser or Playwright's `RequestUtils`/`APIRequestContext` are both cookie-jar clients and are unaffected. Playwright Test's own built-in `webServer.url` readiness poller is **not** — it follows redirects but carries no cookies (confirmed by reading `playwright-core`'s `httpRequest`/`isURLAvailable`) — so it loops forever against a URL affected by this. Fix: `playwright.config.ts`'s `webServer` uses `port`, not `url` — a pure TCP-accept check with no HTTP semantics — and leans on `wait-for-real-readiness.ts`'s own cookie-jar-based poll as the real readiness gate.
- **`@wp-playground/cli`'s `--auto-mount` names the plugin's `wp-content/plugins/` folder after the mounted host directory's basename, not the plugin's real slug.** Inside `tests/e2e/Dockerfile`'s image, the repo is bind-mounted at `/app` (the container's `WORKDIR`), so `--auto-mount` created `wp-content/plugins/app/` — which broke `functional-blueprint.json`'s `activatePlugin` step outright (`Error: ... Plugin /wordpress/wp-content/plugins/app could not be activated`, visible with `--verbosity=debug`). This — not anything Docker- or musl-specific, and not the `--workers` concurrency question below — is what an earlier migration attempt's "unexplained hang immediately after the server reported ready" actually was: the *original* runtime `RequestUtils.activatePlugin()` call (see next bullet) hung silently instead of surfacing this error, since it goes through the REST API rather than the Blueprint. Fix: explicit `--mount` flags for exactly the plugin's own runtime files (main file, `includes/`, `vendor/`, `readme.txt`) at their real slug path, replacing `--auto-mount` entirely.
- **`RequestUtils`'s `rest()` helper (`@wordpress/e2e-test-utils-playwright`) sets no timeout on its underlying `request.fetch()` call, so a single unresponsive request hangs indefinitely** — confirmed by instrumenting the request path end to end (`DEBUG=pw:webserver` plus temporary tracing) after the mount-path fix above still left the suite hanging in exactly the same place, right after `global-setup.ts`'s runtime `requestUtils.activatePlugin(...)` call logged that it had started. `activatePlugin()`'s first step, `getPluginsMap()`, calls `this.rest({ path: '/wp/v2/plugins' })`; that specific GET was observed to never return (not even an error) — reproducible at `--workers=6` and `--workers=1` alike, ruling out cross-worker file-lock contention as the cause. Fix: activate the plugin natively via `functional-blueprint.json`'s `activatePlugin` step (runs server-side at boot, no HTTP round-trip, no untimed REST call in the path) instead of a runtime call — `global-setup.ts` no longer calls `activatePlugin()` at all. `wait-for-real-readiness.ts`'s own poll is unaffected by this same class of bug only because it was given an explicit per-request `{ timeout: 5_000 }` — any future runtime `RequestUtils.rest()`-based call added to this suite should get the same treatment, or use a Blueprint step instead where possible.
- **`@wp-playground/cli` defaults WordPress's own site URL to `http://127.0.0.1:<port>`, not `http://localhost:<port>`.** Both resolve to the same server, but a browser treats them as different origins: admin pages loaded at `localhost` (this suite's `BASE_URL`) had their client-side REST API fetches blocked by CORS against the `127.0.0.1`-rooted API root WordPress advertised (`blocked by CORS policy... Redirect is not allowed for a preflight request`, visible in the browser's own console via Playwright's page-error capture). Fix: `playwright.config.ts`'s `webServer.command` passes `--site-url=${BASE_URL}` explicitly.
- **`@wp-playground/cli server`'s concurrency is sized by `--workers`, unlike wp-now's hardcoded, unconfigurable 5-instance `PHPProcessManager` pool.** Each worker is an independent thread running its own single-instance PHP runtime, so total concurrent request capacity literally equals the worker count — confirmed by reading the installed package source and by triggering the CLI's own boot-time warning at `--workers=2` ("Running fewer than 6 workers may increase the likelihood of deadlock due to workers blocking on file locks"). `playwright.config.ts`'s `webServer.command` pins `--workers=6` explicitly rather than trusting the CLI's own default (`min(6, cpus-1)`): that default computes to only 5 on a 6-CPU host — one below the CLI's own documented safe floor, which 6 treats as a hard minimum, not a ratio to available cores. Kept on those documented merits, but confirmed **not** the cause of the historical hang (reproduced identically at `--workers=1`) — that was the two mount/REST-timeout bugs above. `retries: 2` stays as a general safety net for unrelated flakiness.
- **Residual flakiness after all four fixes above traced to this specific development machine's Docker resource contention, not a further code defect** — evidence: repeated `make test-e2e` runs failed at a *different* point each time (once "Invalid post type" navigating to the CPT admin screen, once stuck after clicking Publish with the trace capturing zero further frames for ~100s), which is the signature of severe, variable resource starvation rather than one deterministic bug. This machine's Docker Desktop VM was shared with ~20 unrelated, already-running containers from a separate project throughout debugging (load average observed swinging between 2.8 and 21+ on a 6-CPU/8GB VM). A clean `make test-e2e` pass on this exact configuration has not yet been observed on this machine; treat a dedicated, uncontended CI runner as the outstanding real verification for this suite.
