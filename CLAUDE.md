...
| `mbgs_request_home_url` (public filter) | `RequestHomeUrl::filter` | brand-aware home URL for server-side consumers; substitutes only on an explicit Brand URL-rule match (never the default-Brand fallback or preview override) and only for canonical `home`/`siteurl` hosts; never downgrades an https input; consumers call `apply_filters( 'mbgs_request_home_url', home_url() )` and must still treat the result as untrusted beyond link/branding use |
