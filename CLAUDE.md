...
- `includes/api.php` — public API functions, required unconditionally from the main plugin file (not PSR-4, global namespace). Currently one: `mbgs_request_home_url( mixed $url ): mixed` — the Brand-aware home URL for server-side consumers (see `Urls/RequestHomeUrl.php` for the transform + gates). Fetches the `request_home_url` service from the Container (fails open to the input when `Plugin::start()` hasn't run) and passes the result through the `mbgs_request_home_url` filter — an extension point applied HERE, not wired in `Plugin::start()`; consumers call the function (guarded by `function_exists()`), they never apply the filter themselves.
...
- `includes/Urls/` — per-Brand URL host rewriting:
  ...
  - `CanonicalAuthority.php` — the install's own authorities (`home` + `siteurl` host[:port], deduped) — i.e. the authorities the plugin is allowed to swap OUT. Single source of truth shared by `HostRewriter` (`all()` — which hosts to find in the HTML) and `RequestHomeUrl` (`matches_host()` — whether the URL handed to the bridge filter is a home URL at all). Port-insensitive on the `matches_host()` path, matching `HostRewriter`'s any-port behavior.
  - `RequestHomeUrl.php` — the transform behind the public `mbgs_request_home_url()` API function (`includes/api.php`): swaps a canonical-home URL's scheme+authority to the browsed authority for server-side consumers (emails, API payloads) that never pass through the buffered egresses. Three gates, all required: the request's Host+path explicitly matched a configured Brand URL rule (`BrandResolver::resolve_current_request_rule_match()` — never the default-Brand fallback or the `?mbgs_preview_brand` override), that Brand has URL rewrite enabled, and the passed URL's host is a `CanonicalAuthority` (it is a home-URL bridge, not a blanket rewriter — a CDN/external URL passes through untouched). The input scheme is a **floor**: force-https/`is_ssl()` can upgrade http→https, but an https home URL is never downgraded (a TLS-terminating proxy that leaves `is_ssl()` false would otherwise turn a correct https reset link into http). Typed `mixed`, not `string` — a public API takes whatever consumers pass, and a `string` parameter would fatal on null/array and coerce `false` to `''` instead of failing open. A protocol-relative input becomes absolute (deliberate divergence from `HostRewriter`, which preserves the form — a schemeless URL is useless in an email). The substituted authority derives from the client-supplied Host header — it is only ever a host that explicitly matched a configured Brand URL rule, but consumers must still treat it as untrusted for anything beyond link/branding selection.
  - `HostRewriter.php` — the LAST PageBuffer transformer: for Brands with the option on, swaps the canonical `home`/`siteurl` authority (host[:port]) in the final HTML for the authority being browsed — absolute, protocol-relative, and JSON-escaped forms; path/query never touched. Also keeps server-side redirects on the browsed host: filters `redirect_canonical` so core can't bounce visitors back to the canonical host (returns false when only the host differed), adds the browsed host to `allowed_redirect_hosts` so `wp_validate_redirect()` stops rejecting brand-host targets (WooCommerce/wp-login `redirect_to` flows), and rewrites canonical-host `Location:` targets on the `wp_redirect` filter — with a GET-self-redirect guard so `HostCanonicalizer`'s www↔apex 301 survives (see Gotchas). Served REST payloads get the same rewrite on `rest_pre_echo_response` (GET/HEAD only, never `context=edit`) — HTML fragments fetched over REST (infinite scroll, AJAX card batches) never pass through the PageBuffer, and this egress filter fires after `_links`/`_embedded` are merged, which `rest_post_dispatch` would miss. Sitemap XML served by The Another SEO is a third buffer-blind egress: that plugin serves on `template_redirect` at priority 0 and exits before the buffer opens, so `filter_taseo_sitemap_xml` subscribes to its `taseo_sitemap_xml` filter (string-guarded, delegates to `replace()`; inert when the plugin is absent).
...
| `rest_pre_echo_response` | `HostRewriter::filter_rest_pre_echo_response` | rewrite canonical-host URLs in served REST payloads (fragment endpoints; GET/HEAD reads, never `context=edit`) |
| `taseo_sitemap_xml` | `HostRewriter::filter_taseo_sitemap_xml` | rewrite canonical-host URLs in sitemap XML served by The Another SEO (its serve path exits before the PageBuffer opens; inert when that plugin is absent) |
...
| `mbgs_request_home_url` (extension filter, applied in `includes/api.php` — NOT wired here) | third parties may `add_filter` | overrides the value computed by the public `mbgs_request_home_url()` API function; consumers call the function (with a `function_exists()` guard), never `apply_filters` |
