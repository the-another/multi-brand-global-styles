...
- **`@wp-playground/cli server`'s concurrency is sized by `--workers`, unlike wp-now's hardcoded, unconfigurable 5-instance `PHPProcessManager` pool.** Each worker is an independent thread running its own single-instance PHP runtime, so total concurrent request capacity literally equals the worker count — confirmed by reading the installed package source and by triggering the CLI's own boot-time warning at `--workers=2` ("Running fewer than 6 workers may increase the likelihood of deadlock due to workers blocking on file locks"). `playwright.config.ts`'s `webServer.command` pins `--workers=6` explicitly rather than trusting the CLI's own default (`min(6, cpus-1)`): that default computes to only 5 on a 6-CPU host — one below the CLI's own documented safe floor, which 6 treats as a hard minimum, not a ratio to available cores. `retries: 2` stays as a general safety net for unrelated flakiness. **Caveat:** `--workers=6` is a defensible, evidence-based fix, but is not independently confirmed to resolve a real hang — `make test-e2e` reproduced the same hang with and without it on a heavily-oversubscribed local Docker host (~20 unrelated containers, load average 15-21+), which points at least as strongly at host contention as at worker count. Treat a clean CI run (a dedicated, uncontended runner) as the outstanding real verification for this suite, not yet observed.
