...
- `includes/Urls/` — per-Brand URL host rewriting:
  ...
  - `RequestHomeUrl.php` — callback behind the public `mbgs_request_home_url` bridge filter: swaps a canonical-home URL's scheme+authority to the browsed authority for server-side consumers (emails, API payloads) that never pass through the buffered egresses. Same gating as `HostRewriter` (Brand resolved + URL rewrite enabled; scheme honors force-https); fails open.
...
| `mbgs_request_home_url` (public filter) | `RequestHomeUrl::filter` | brand-aware home URL for server-side consumers; consumers call `apply_filters( 'mbgs_request_home_url', home_url() )` |
