...
- `includes/Urls/` — per-Brand URL host rewriting:
  ...
  - `RequestHomeUrl.php` — callback behind the public `mbgs_request_home_url` bridge filter: swaps a canonical-home URL's scheme+authority to the browsed authority for server-side consumers (emails, API payloads) that never pass through the buffered egresses. Substitutes only when the request's Host+path explicitly matched a configured Brand URL rule (`BrandResolver::resolve_current_request_rule_match()`) — never on the default-Brand fallback or the `?mbgs_preview_brand` override — and that Brand has URL rewrite enabled; scheme honors force-https; fails open. The substituted authority derives from the client-supplied Host header — it is only ever a host that explicitly matched a configured Brand URL rule, but consumers must still treat it as untrusted for anything beyond link/branding selection.
...
| `mbgs_request_home_url` (public filter) | `RequestHomeUrl::filter` | brand-aware home URL for server-side consumers; substitutes only on an explicit Brand URL-rule match (never the default-Brand fallback or preview override); consumers call `apply_filters( 'mbgs_request_home_url', home_url() )` and must still treat the result as untrusted beyond link/branding use |
