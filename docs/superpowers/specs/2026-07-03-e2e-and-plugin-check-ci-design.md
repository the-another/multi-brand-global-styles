...
- **Functional suite:** new `tests/e2e/functional-blueprint.json`, passed to `wp-now start --blueprint=...`. It sets the permalink structure via Blueprint steps instead of the current mu-plugin (`e2e-environment.php` copied into the shared, machine-global `~/.wp-now/mu-plugins/` directory — a real correctness risk if anything else on the same machine/CI runner uses wp-now concurrently, and it leaks state across runs if teardown fails). `global-setup.ts`/`global-teardown.ts` lose the mu-plugin copy/cleanup logic entirely. Exact Blueprint step name(s) for permalink structure must be confirmed against the installed `@wp-playground/blueprints` schema during implementation; if no direct step exists, a `runPHP` step running the same rewrite-flush code is the fallback — still eliminates the shared-directory hack either way.
- **Plugin Check suite:** `check-plugin-blueprint.json` gains an explicit `login` entry. `check-plugin-global-setup.ts` drops the manual `wp-login.php` form POST in favor of reading back whatever session the Blueprint-driven login/`--login` flag establishes. `waitForRealReadiness` is still called first, since it's about server startup ordering, not login mechanics. If the CLI's `--login` flag cookie isn't independently inspectable/reusable for Playwright's storage state, keep a minimal fallback POST — this is an implementation-time empirical check, not a design assumption to build on faith.
